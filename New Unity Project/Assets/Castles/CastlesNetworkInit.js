...
function RequestPlayer (name: String, r: float, g: float, b: float, info: NetworkMessageInfo) {
	...
	if (Network.isServer && playerCount == maxPlayers)
	{
		...
		for (var receiver : CastlePlayer in playerList) {
			for (var playerInformation : CastlePlayer in playerList) {
				if (receiver.player != playerInformation.player && receiver.player.ToString() != "0") {
					Debug.Log("Sending info on player " + playerInformation.player + " to player " + receiver.player);
					networkView.RPC("ReceivePlayer", receiver.player, playerInformation.number, playerInformation.color.r, playerInformation.color.g, playerInformation.color.b, playerInformation.name, playerInformation.player);
				}
			}
		}
